...
export const zhTranslations: Record<string, string> = {
  ...
  // AI API
  'AI API': 'AI API',
  'Configure AI API for features (commit message, branch name, code review)':
    '配置 AI API 用于各功能（commit 消息、分支名称、代码审查）',
  'Configure AI API providers for AI features': '配置 AI API 提供方用于 AI 功能',
  'HTTP API protocol': 'HTTP API 协议',
  Protocol: '协议',
  'Test Connection': '测试连接',
  Connected: '连接成功',
  'Not configured': '未配置',
  Mode: '调用方式',
  'Choose between local CLI providers and configured HTTP APIs':
    '选择使用本地 CLI 提供方或已配置的 HTTP API',
  'OpenAI Compatible': 'OpenAI 兼容',
  Anthropic: 'Anthropic',
  'Fetch models': '获取模型',
  'Fetching...': '获取中...',
  'No models found': '未找到模型',
  'Search or enter model name...': '搜索或输入模型名称...',
  'Default Model': '默认模型',
  'Used when a feature does not override the API model':
    'API 模式下各功能默认使用该模型，下方各模块可单独覆盖',
  'Default (use global config)': '默认（使用全局配置）',
  'API Model': 'API 模型',
  'Leave empty to use the global default model': '留空则使用全局默认模型',
  'API endpoint base URL, including /v1 when required': 'API 端点基础 URL，需要时包含 /v1',
  'Used when any feature runs in API mode': '任意功能使用 API 模式时使用该密钥',
  'API Key': 'API 密钥',
  // Commit Message Generator
  'Commit Message Generator': '生成 Commit Message',
  'Auto-generate commit messages using AI': '使用 AI 自动生成 commit 消息',
  ...
  'Auto-generate branch names using AI': '使用 AI 自动生成分支名称',
